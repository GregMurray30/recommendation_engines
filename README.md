...
**Figure 3:** *Examples of weighted rating-difference (x axis) Gaussian distributions for three different user pairs of varying similarity and variance. User pair 1-2 is most probable to be similar, followed by user pair 1-4, then user pair 1-3 with the least probability of being similar. If the threshold paramater θ were set at 1, then each user pair's edge distance would be the area under that pair's bell curve to the right of the pink dashed line (probability they are **not** similar since a longer edge distance means less similar node pairs).*
